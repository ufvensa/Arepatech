...
function SourceLink({ source }) {
  let safe = false;
  try {
    safe = new URL(source.source_url).protocol === "https:";
  } catch {
...
function Workspace() {
  ...
  return (
    <main className="finance">
      ...
      {data && (
        <>
          {tab === "Overview" && <Overview summary={summary} data={data} />}
          {tab === "Records" && <Records data={data} reload={reload} />}
          {tab === "Drive imports" && (
            <DriveImports data={data} reload={reload} />
          )}
    ...
    <>
      ...
      <p className="finance-note">
        ...
        {summary.cash_snapshot
          ...
          : " (not yet recorded)"}{" "}
        plus recorded movements after that day. SG allocations are tracked
        separately.
      </p>
...
function DriveImports({ data, reload }) {
